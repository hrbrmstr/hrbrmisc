add_subtitle <- function() {
  ...
  ui <- miniPage(
    ...
    miniContentPanel(
      ...
      fillRow(width="100%", height="9%",
              ...
              numericInput("lineheight", "Line height", value="1", min=0, width="100px")
      ),
  ...
  server <- function(input, output, session) {
    ...
    observeEvent(input$done, {
      ...
      command <- sprintf("ggplot_with_subtitle(%s, %s, %s, %s, %s, %s, %s, %s)\n",
                         rstudio_input_text,
                         "label=subst",
                         sprintf("fontfamily='%s'", input$fontfamily),
                         sprintf("fontsize='%s'", input$fontsize),
                         sprintf("hjust='%s'", input$hjust),
                         sprintf("vjust='%s'", input$vjust),
                         sprintf("bottom_margin='%s'", input$bottom_margin),
                         sprintf("lineheight='%s'", input$lineheight))

      command <- formatR::tidy_source(text=command, output=FALSE,
                                      width.cutoff=40)$text.tidy

      rstudioapi::insertText(paste(command, collapse = "\n"))
